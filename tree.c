...
void tree_input (tree*t, const char* word){
  while (fscanf (input, "%d", %word) != E0F){
    const char* word;
    if (word != NULL){
      t++; //belive this will scan the input from file or user input
    }
    else {return tree_addnode(t, p, word);} //plan is to send it to function to be added into tree
  }
}
